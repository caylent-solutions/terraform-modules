...
# AWS Parameters and Secrets Lambda Extension constants
# These are AWS-defined values that change only when AWS releases a new extension version.
# Override only when AWS releases a newer version not yet reflected in the default.
variable "ext_layer_version" {
  description = "AWS Parameters and Secrets Lambda Extension layer version number"
  type        = number
  default     = 11
}

variable "ext_layer_arn_pattern_x86_64" {
  description = "ARN format pattern for the x86_64 Parameters and Secrets Lambda Extension layer"
  type        = string
  default     = "arn:aws:lambda:%s:%s:layer:AWS-Parameters-and-Secrets-Lambda-Extension:%d"
}

variable "ext_layer_arn_pattern_arm64" {
  description = "ARN format pattern for the arm64 Parameters and Secrets Lambda Extension layer"
  type        = string
  default     = "arn:aws:lambda:%s:%s:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:%d"
}

variable "ext_env_var_http_port" {
  description = "Environment variable name for the extension HTTP port"
  type        = string
  default     = "PARAMETERS_SECRETS_EXTENSION_HTTP_PORT"
}

variable "ext_env_var_cache_enabled" {
  description = "Environment variable name for enabling the extension cache"
  type        = string
  default     = "PARAMETERS_SECRETS_EXTENSION_CACHE_ENABLED"
}

variable "ext_env_var_cache_size" {
  description = "Environment variable name for the extension cache size"
  type        = string
  default     = "PARAMETERS_SECRETS_EXTENSION_CACHE_SIZE"
}

variable "ext_env_var_max_connections" {
  description = "Environment variable name for the extension max connections"
  type        = string
  default     = "PARAMETERS_SECRETS_EXTENSION_MAX_CONNECTIONS"
}

variable "ext_env_var_secrets_timeout" {
  description = "Environment variable name for the Secrets Manager timeout"
  type        = string
  default     = "SECRETS_MANAGER_TIMEOUT_MILLIS"
}

variable "ext_env_var_ssm_timeout" {
  description = "Environment variable name for the SSM Parameter Store timeout"
  type        = string
  default     = "SSM_PARAMETER_STORE_TIMEOUT_MILLIS"
}
