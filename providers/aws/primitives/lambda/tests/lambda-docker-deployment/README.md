...
Docker deployment tests validate features specific to Lambda functions deployed using container images from ECR. These tests cover ECR integration, EFS mounts, ARM64 architecture, and custom Lambda layers.
...
1. **Provision Once**: Deploy `lambda-docker-deployment` example infrastructure (ECR, Docker build/push, EFS, Lambda)
...
WARNING: Docker tests take 70+ minutes due to EFS, Docker image build, and VPC ENI cleanup. Recommended for manual testing only.
...
#### 5. FunctionArm64Architecture
**Purpose**: Validates ARM64 (Graviton2) architecture

**Validates**:
- `function_architecture` - Architecture output equals `arm64`
...
```bash
go test -v -run TestDockerDeploymentFeatures/EFSConfiguration ./tests/lambda-docker-deployment/
```
...
=== RUN   TestDockerDeploymentFeatures/FunctionArm64Architecture
--- PASS: TestDockerDeploymentFeatures (1800.00s)
    ...
    --- PASS: TestDockerDeploymentFeatures/FunctionArm64Architecture (0.01s)
PASS
```
...
- **Total Execution Time**: ~70+ minutes (includes 25-30 min provision, testing, and 45+ min destroy due to VPC ENI cleanup)
...
├── FunctionArm64Architecture (subtest)
...
**Note**: Docker tests take ~70+ minutes due to EFS, Docker image build, and VPC ENI cleanup during destroy.
